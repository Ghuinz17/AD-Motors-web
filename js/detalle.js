...
async function loadVehiculo() {
  try {
    ...
    imagenes = (data.vehiculo_imagenes || []).map((i) => {
      const { data: u } = db.storage
        .from("ad-motors-images")
        .getPublicUrl(i.imagen);
      return u.publicUrl;
    });
...
function renderPage() {
  ...
  const specs = [
    { icon: "fa-calendar", label: "Año", value: vehiculo.ano_fabricacion },
    {
      icon: "fa-gauge-high",
      label: "Kilometraje",
      value: vehiculo.kilometraje?.toLocaleString("es-ES") + " km",
    },
    {
      icon: "fa-gas-pump",
      label: "Combustible",
  ...
  if (vehiculo.reservado) {
    ...
    document
      .getElementById("btnRealizarCompra")
...
function prevImage() {
  setImage((currentImg - 1 + imagenes.length) % imagenes.length);
}
function nextImage() {
  setImage((currentImg + 1) % imagenes.length);
}
  ...
  try {
    ...
    try {
      ...
      await emailjs.send(EMAILJS_SERVICE_ID, EMAILJS_TEMPLATE_COMPRA, {
        ...
        mensaje:
          "Hemos registrado tu reserva correctamente. En breve nos pondremos en contacto contigo para coordinar todos los detalles.",
    ...
    closeModal("modalPasarela");
    showToast("¡Reserva realizada! Elige cómo continuar", "success");
    // Abrir modal para acordar cita o día de pago
    setTimeout(() => openModal("modalCitaReserva"), 400);
  } catch (err) {
...
function clearAllFieldErrors() {
  ["visitaNombre", "visitaTelefono", "visitaFecha", "visitaHora"].forEach(
    clearFieldError,
  );
}
...
async function handleSubmitVisita(e) {
  ...
  if (!nombre || nombre.length < 2) {
    showFieldError("visitaNombre", "Introduce el nombre del asistente");
    ok = false;
  }
  if (!tel || !/^[0-9+\s]{9,15}$/.test(tel)) {
    showFieldError("visitaTelefono", "Introduce un teléfono válido");
    ok = false;
  }
  if (!fecha) {
    showFieldError("visitaFecha", "Selecciona una fecha");
    ok = false;
  } else {
    const hoy = new Date();
    ...
    if (fecha < minStr) {
      showFieldError("visitaFecha", "La fecha debe ser a partir de mañana");
      ok = false;
    }
  }
  if (!hora) {
    showFieldError("visitaHora", "Selecciona una hora");
    ok = false;
  }
  if (!ok) return;
  ...
  try {
    ...
    try {
      const result = await emailjs.send(
        EMAILJS_SERVICE_ID,
        EMAILJS_TEMPLATE_VISITA_CLIENTE,
        {
          to_email: currentUser.email,
          ...
          mensaje:
            "Hemos recibido tu solicitud para ver el vehículo en persona. Nos pondremos en contacto contigo a la mayor brevedad posible para confirmar los detalles.",
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  document
    .getElementById("btnRealizarCompra")
    ?.addEventListener("click", handleRealizarCompra);
  ...
  // Formulario fecha de pago
  document
    .getElementById("formFechaPago")
    ?.addEventListener("submit", async (e) => {
      e.preventDefault();
      const fecha = document.getElementById("fechaPagoFecha")?.value;
      const hora = document.getElementById("fechaPagoHora")?.value;
      const tel = document.getElementById("fechaPagoTel")?.value.trim();
      ...
      if (!fecha || fecha < minStr) {
        document.getElementById("fechaPagoFechaError").textContent =
          "Selecciona una fecha a partir de mañana";
        document.getElementById("fechaPagoFechaError").classList.add("show");
        ok = false;
      } else {
        document.getElementById("fechaPagoFechaError").classList.remove("show");
      }
      if (!hora) {
        document.getElementById("fechaPagoHoraError").textContent =
          "Selecciona una hora";
        document.getElementById("fechaPagoHoraError").classList.add("show");
        ok = false;
      } else {
        document.getElementById("fechaPagoHoraError").classList.remove("show");
      }
      if (!tel || !/^[0-9+\s]{9,15}$/.test(tel)) {
        document.getElementById("fechaPagoTelError").textContent =
          "Introduce un teléfono válido";
        document.getElementById("fechaPagoTelError").classList.add("show");
        ok = false;
      } else {
        document.getElementById("fechaPagoTelError").classList.remove("show");
      }
      if (!ok) return;

      const btn = document.getElementById("btnConfirmarFechaPago");
      setLoading(btn, true);
      try {
        // Guardar como solicitud de tipo pago
        await db.from("solicitudes_revision").insert([
          {
            id_vehiculo: vehiculoId,
            id_usuario: currentUser?.id || null,
            marca_modelo: `${vehiculo.marca} ${vehiculo.modelo}`,
            nombre_asistente: currentUser?.email || "",
            telefono: tel,
            fecha_visita: fecha,
            hora_visita: hora,
          },
        ...
        showToast(
          "Cita de pago registrada. Te contactaremos para confirmar.",
          "success",
        );
  ...
  // Fecha mínima en formulario de pago
  const hoyP = new Date();
  const mananaP = new Date(
    hoyP.getFullYear(),
    hoyP.getMonth(),
    hoyP.getDate() + 1,
  );
  const minPagoStr = `${mananaP.getFullYear()}-${String(mananaP.getMonth() + 1).padStart(2, "0")}-${String(mananaP.getDate()).padStart(2, "0")}`;
  const fechaPagoInp = document.getElementById("fechaPagoFecha");
  if (fechaPagoInp) fechaPagoInp.min = minPagoStr;
  document
    .getElementById("btnSolicitarVisita")
    ?.addEventListener("click", handleSolicitarVisita);
  document
    .getElementById("btnConfirmarCompra")
    ?.addEventListener("click", confirmarCompra);
  document
    .getElementById("formVisita")
    ?.addEventListener("submit", handleSubmitVisita);
  document
    .getElementById("galleryMainImg")
