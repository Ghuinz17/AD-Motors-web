...
async function initPerfil() {
  ...
  try {
    ...
    /* RLS o tabla vacía — usar metadata */
  }
...
async function loadPedidos(userId) {
  ...
  cont.innerHTML = pedidosConDetalle
    .map((p) => {
...
async function handleUpdateEmail(e) {
  ...
  if (newEmail === user.email) {
    showToast("El email es el mismo que el actual", "error");
    return;
  }
  ...
  try {
    ...
    if (error) throw error;
    showToast(
      "Revisa tu bandeja de entrada — se ha enviado un enlace de confirmación al nuevo correo",
      "success",
    );
  } catch (err) {
