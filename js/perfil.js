...
async function initPerfil() {
  ...
  // Si no existe fila en usuario, crearla con datos de user_metadata
  if (!userData) {
    const nombre = user.user_metadata?.nombre || user.email?.split('@')[0] || '';
    const tel    = user.user_metadata?.phone  || '';
    ...
    // Recargar tras insertar
    const { data: newData } = await db.from('usuario').select('*').eq('id_usuario', user.id).single();
    Object.assign(userData || {}, newData);
    userData = newData;
  }

  // Nombre: tabla usuario → user_metadata → parte del email
