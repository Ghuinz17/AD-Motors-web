...
async function handleSoporte(e) {
  ...
  let ok = true;
  if (!asunto)      { showSoporteError('soporteAsunto',      'Introduce un asunto');          ok = false; }
  if (!descripcion || descripcion.length < 10)
                    { showSoporteError('soporteDescripcion', 'Describe el problema (mín. 10 caracteres)'); ok = false; }
  if (!email || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email))
                    { showSoporteError('soporteEmail',       'Introduce un email válido');    ok = false; }
  if (!ok) return;
