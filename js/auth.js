...
function validateRegister() {
  ...
  if (!email||!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) { showError('regEmail','Email no válido'); ok=false; }
...
function validateLogin() {
  ...
  if (!email||!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) { showError('loginEmail','Email no válido'); ok=false; }
...
async function handleRegister(e) {
  ...
  try {
    ...
    pendingEmail = email;
    if (data.user) {
      await db.from('usuario').upsert({ id_usuario: data.user.id, nombre, email, phone: tel });
    }
    showOtpStep();
