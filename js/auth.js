...
const COMMON_DOMAINS = [
  ...
  'hotmail.es','outlook.es','yahoo.es','gmail.es', "g.educaand.es",
];
...
function validateRegister() {
  ...
  if (!nombre||nombre.length<2)             { showError('regNombre','Introduce tu nombre completo'); ok=false; }
  if (!email||!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) {
    showError('regEmail','Email no válido'); ok=false;
  } else {
  ...
  if (!telefono||!/^[0-9+\s]{9,15}$/.test(telefono))     { showError('regTelefono','Teléfono no válido (mín. 9 dígitos)'); ok=false; }
  if (!pw)                                  { showError('regPassword','Introduce una contraseña'); ok=false; }
  else if (!checkPassword(pw).allMet)       { showError('regPassword','La contraseña no cumple todos los requisitos'); ok=false; }
  if (pw!==pw2)                             { showError('regPassword2','Las contraseñas no coinciden'); ok=false; }
  if (!terms)                               { showToast('Debes aceptar los términos y condiciones','error'); ok=false; }
  return ok;
}
...
function validateLogin() {
  ...
  if (!email||!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) {
    showError('loginEmail','Email no válido'); ok=false;
  } else {
    const suggestion = checkEmailTypo(email);
    if (suggestion) {
      const user = email.split('@')[0];
  ...
  if (!pw) { showError('loginPassword','Introduce tu contraseña'); ok=false; }
  return ok;
}
...
async function handleRegister(e) {
  ...
  try {
    ...
    if (err.message?.includes('already registered')) showError('regEmail','Este email ya está registrado');
...
async function handleLogin(e) {
  ...
  try {
    ...
    if (err.message?.includes('Invalid') || err.message?.includes('invalid')) showError('loginPassword','Email o contraseña incorrectos');
    else if (err.message?.includes('Email not confirmed')) { pendingEmail = email; showOtpStep(); showToast('Confirma tu email primero','info'); }
    else showToast('Error: ' + err.message, 'error');
  } finally { setLoading(btn, false); }
}
...
function initOtpInputs() {
  ...
  inputs.forEach((inp, i) => {
    ...
    inp.addEventListener('paste', e => {
      ...
      paste.split('').forEach((c,j) => { if(inputs[j]){inputs[j].value=c;inputs[j].classList.add('filled');} });
...
async function handleVerifyOtp(e) {
  ...
  setLoading(btn, true);
  try {
    const { data, error } = await db.auth.verifyOtp({ email: pendingEmail, token: code, type: 'email' });
    if (error) throw error;
...
async function handleResendOtp() {
  ...
  try {
    ...
    const iv = setInterval(() => {
      s--; btn.textContent = `Reenviar (${s}s)`;
      if (s<=0) { clearInterval(iv); btn.textContent='Reenviar código'; btn.disabled=false; }
    }, 1000);
  } catch { showToast('Error al reenviar','error'); btn.disabled=false; }
}
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.getElementById('formOtp')?.addEventListener('submit', handleVerifyOtp);
  ...
  document.querySelectorAll('.form-input').forEach(inp => inp.addEventListener('input', () => clearError(inp.id)));
